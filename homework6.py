...
print('PROBLEM 1: Confidence Interval')
print('Projected average reward', multiGame.get_overall_mean_reward())
print('95% projection interval of average reward', multiGame.get_ci_mean_reward(alpha=0.05))
print('Projected average loss probability', multiGame.get_overall_money_lost())
print('95% projection interval of average loss probability', multiGame.get_ci_money_lost(alpha=0.05))

print('PROBLEM 2: Interpretation of Confidence Intervals')
print('If you were to repeat this simulation many times to construct a large number of independent 95% confidence\n '
      'intervals, the proportion of these confidence intervals that will cover the true means is .95')

print('PROBLEM 3: Casino Owner versus Gamblers')
NUM_SIM_GAMES_OWNER = 10
N_GAMES_OWNER = 100
ownerGame = MultiSetofGames(ids=range(NUM_SIM_GAMES_OWNER),
                             n_games=[N_GAMES_OWNER]*NUM_SIM_GAMES,
                            prob_head=[PROB_HEAD]*NUM_SIM_GAMES)
ownerGame.simulate()
print('The expected reward for the casino owner is', ownerGame.get_overall_mean_reward() *-1)
print('The uncertainty of these estimates is between', ownerGame.get_ci_mean_reward(alpha=0.05), 'this means that if\n '
      'you were to repeat this game many times, 95% of the time the true average reward will fall within this interval.\n'
      'This is from the perscpetive of the game player so from the perspective of the casino owner these values will \n'
      'actually be positive.')

NUM_SIM_GAMES_GAMBLER = 2
N_GAMES_GAMBLER = 10
gamblerGame = MultiSetofGames(ids=range(NUM_SIM_GAMES_GAMBLER),
                              n_games=[N_GAMES_GAMBLER]*NUM_SIM_GAMES,
                              prob_head=[PROB_HEAD]*NUM_SIM_GAMES)
gamblerGame.simulate()
print('The expected reward for the gambler is', gamblerGame.get_overall_mean_reward())
print('The uncertainty of these estimates is between', ownerGame.get_pi_mean_reward(alpha=0.05), 'this means that if\n'
      'you were to repeat this game, your next reward will fall have a 95% probability of falling within this range')
